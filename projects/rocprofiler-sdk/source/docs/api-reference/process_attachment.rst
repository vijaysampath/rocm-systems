...
When attachment is enabled, it takes precedence over an implicitly discovered
``rocprofiler_configure`` symbol. This allows framework libraries to expose
dormant profiler entry points without preventing the attachment listener from
starting.

Explicit startup profiling takes precedence over attachment. It is selected by
``ROCPROFILER_REGISTER_FORCE_LOAD=1`` or by an ``LD_PRELOAD`` library that
directly exports ``rocprofiler_configure``. ``ROCP_TOOL_LIBRARIES`` and
``ROCPROFILER_REGISTER_LIBRARY`` also select startup profiling by default, but
``ROCPROFILER_REGISTER_FORCE_LOAD=0`` prevents those two variables from
triggering startup loading. A warning indicates which mode was selected when
startup profiling and attachment conflict.
