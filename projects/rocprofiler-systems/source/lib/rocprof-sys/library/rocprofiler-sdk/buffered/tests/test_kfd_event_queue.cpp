...
// on_kfd_event_queue_configure<Externals> calls exactly these three members
...
// and on_kfd_event_queue_configure. The on_records-only members
     ...
     on_configure_registers_category_string_and_skips_pmc_info_without_gpu_agents)
{
    ...
    EXPECT_CALL(*g_externals_mock,
                add_string(Eq(externals::kfd_event_queue_category_name)))
        .Times(1);
...
TEST(kfd_event_queue_test, on_configure_registers_pmc_info_for_each_gpu_agent)
{
    ...
    auto gpu_agent = std::make_shared<agent_t>(
        agent_t{ .type = externals::AGENT_TYPE_GPU, .device_type_index = 2 });
        ...
        *g_externals_mock,
        add_pmc_info(AllOf(
            ...
            Field(&pmc_info_data_t::name,
                  Eq(std::string{ externals::kfd_event_queue_category_name })),
            Field(&pmc_info_data_t::symbol,
                  Eq(std::string{ "KFD Event Queue Operations" })),
            Field(&pmc_info_data_t::description,
                  Eq(std::string{ externals::kfd_event_queue_category_description })))))
        .Times(1);
