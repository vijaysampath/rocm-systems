...
public:
  ...
  /// that has no mapping yet. Existing mapped-page clipping remains the legacy
  /// contract: the mapped prefix is copied, the remainder is zero-filled, and
  /// the page-bounded operation completes.
  ...
  /// sparse fallback while preserving the legacy mapped-prefix behavior for a
  /// page-table entry whose host extent is deliberately clipped.
  ...
  [[nodiscard]] AccessOutcome atomic_modify_strict(uint64_t addr, uint32_t size,
                                                   const std::function<void(uint8_t *)> &mutation,
                                                   uint32_t vmid) {
    const FaultScope faults;
    auto discarded = [](uint8_t *) {};
    atomic_rmw_impl(addr, size, mutation, discarded, vmid);
  ...
  bool copy_mapped_span(uint64_t addr, void *bytes, size_t size, uint32_t vmid,
    ...
    return with_page_mapping(addr, vmid, [&](const LegacyPageTableEntry *pte, IdentityPage page) {
      ...
      if (addr >= kUserSpaceLimit || size > kUserSpaceLimit - addr)
        return false;
      const bool moved =
          into_memory ? page.write(page_offset, bytes, size) : page.read(page_offset, bytes, size);
      if (!moved)
        note_rejected_identity_access(
            addr, vmid, into_memory ? page.write_refusal_cause() : MemoryFaultCause::NotPresent);
      return moved;
    });
  }
